import { ensureId, toError, checkStatus } from "../api-utils";
...
export const updateAiIdea = async (ideaId: string, updates?: Partial<ideaType>): Promise<ideaType> => {
  ensureId(ideaId);
  try {
    const res = await api.patch<ideaType>(`${Ai_Idea_BASE}/${ideaId}`, updates);
    checkStatus(res.status);
    return res.data;
  } catch (err) {
    throw toError(err);
