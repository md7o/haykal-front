        ...
        >
          {[
            ...
            <div key={itm.label} className="bg-card-bg rounded-base p-3 w-full text-center">
              <div className={`${view === "mobile" ? "text-xl" : "text-2xl"} font-semibold text-title`}>
                {String(itm.value).padStart(2, "0")}
              </div>
              <div className="text-description text-xs">{itm.label}</div>
            </div>
          ))}
        </div>
      ) : (
        <div className={`${isColumn ? "justify-center" : ""} flex items-baseline text-title font-semibold`}>
