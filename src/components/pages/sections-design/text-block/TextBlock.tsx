...
export default function TextBlock({ config, view = "desktop" }: { config: TextBlockConfig; view?: "desktop" | "mobile" }) {
  const { title, description, alignment = "center", style = "without-background" } = config;
  ...
  return (
    <section className={`${containerWidth} mx-auto`}>
      <div className={`${wrapperClass} ${alignClass}`}>
        {title ? <h2 className={`${titleSize} font-semibold text-title mb-3 break-words whitespace-normal`}>{title}</h2> : null}
        {description ? <p className={`${descSize} text-description break-words whitespace-normal`}>{description}</p> : null}
      </div>
    </section>
  );
}
