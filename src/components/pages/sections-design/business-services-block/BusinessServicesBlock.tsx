...
function ServiceCard({ item, view }: { item: ServiceItem; view: "desktop" | "mobile" }) {
  ...
  const Text = (
    <div className={`space-y-2 ${isColumn ? "text-center" : "text-left"}`}>
      <h3 className={`${titleSize} font-bold text-title break-words`}>{item.title}</h3>
      {item.description ? <p className={`${descSize} text-description break-words`}>{item.description}</p> : null}
      {item.ctaLabel ? (
        item.ctaLink ? (
