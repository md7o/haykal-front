...
export default function SectionsSidebar({ onEdit, onSelectToggle, selectedSectionId }: SectionsSidebarProps) {
  const { used, available, addSection, removeSection, reorderUsed, updateSectionConfig } = useStudio();
    ...
    <>
            ...
            <div className="group p-3 rounded-lg bg-card-bg">
              <div className="flex items-center ">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (header) {
                      removeSection(header.id);
                    } else {
                      addSection("header");
                    }
                  }}
                  aria-label={header ? "Remove header" : "Add header"}
                >
                  {header ? <MinusCircle className="w-4 h-4 text-red-700" /> : <PlusCircle className="w-4 h-4 text-green-700" />}
                </Button>
      ...
      <SidebarGroup>
        ...
        <SidebarGroupContent>
          ...
          >
            <SortableContext
              items={used.filter((u: any) => u.type !== "header").map((u: any) => u.id)}
      ...
      <SidebarGroup>
        ...
        <SidebarGroupContent>
          <div className="space-y-2">
            {available
              .filter((a: { type: string }) => a.type !== "header")
              .map((a: { type: string; label: string }) => (
                <AvailableItem
                  key={a.type}
