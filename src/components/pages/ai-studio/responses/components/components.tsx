...
import { ideaType } from "@/api/ai-api/idea-endpoints";
...
export const SubsectionHeader = memo(({ title, variant = "default" }: { title: string; variant?: "default" | "accent" }) => (
  <h3 className={variant === "accent" ? " text-sm text-accent" : " text-title"}>{title}</h3>
));
SubsectionHeader.displayName = "SubsectionHeader";

export const CompetitorCard = memo(({ competitor }: { competitor: any }) => (
...
export const TechCard = memo(({ label, item }: { label: string; item: any }) => (
  <div className={CARD_STYLES.default}>
...
export const RoleItem = memo(({ role }: { role: any }) => (
...
export const PhaseItem = memo(({ item }: { item: any }) => (
  <div className={CARD_STYLES.default}>
    <RenderContent item={item} />
    Technical
  </div>
...
export const RiskItem = memo(({ label, item }: { label: string; item: any }) => (
  <div>
    <h3 className="font-semibold text-accent mb-2">{label}</h3>
    <RenderContent item={item} />
  </div>
));
...
// ===== Data Components =====
export const ProjectDetails = memo(({ details }: { details: Record<string, string> }) => {
...
export const RawDataViewer = memo(({ data }: { data: ideaType }) => (
...
RawDataViewer.displayName = "RawDataViewer";
